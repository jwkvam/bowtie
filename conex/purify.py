...
def _purify(f, stoplist=[], verbose=False):
    if verbose:
...
_purify = _purify(_purify) # optimize thyself!

def bind_all(mc, stoplist=[],  verbose=False):
    ...
    def _bind_all(mc, stoplist=[],  verbose=False):

      if isinstance(mc, (ModuleType, type)):
        for k,v in list(vars(mc).items()):
          if type(v) is FunctionType:
              newv = _purify(v, stoplist,  verbose)
              setattr(mc, k, newv)
          elif isinstance(v, type):
              _bind_all(v, stoplist, verbose)
...
@_purify
def purify(stoplist=[], verbose=False):
    ...
    """
    if type(stoplist) == type(purify):
        raise ValueError("The bind_constants decorator must have arguments.")
    return lambda f: _purify(f, stoplist, verbose)
